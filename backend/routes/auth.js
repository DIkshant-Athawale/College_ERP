...
router.get("/me", authenticate, async (req, res) => {
  try {
    ...
    if (userType === "student") {
      const [rows] = await pool.execute(
        `SELECT student_id AS id, first_name, last_name, email, department_id, year, semester
         FROM students WHERE student_id = ?`,
        [userId]
      );
      if (rows.length > 0) profile = { ...rows[0], role: "student", userType: "student" };
    } else {
      // faculty or admin
      const [rows] = await pool.execute(
        `SELECT teacher_id AS id, first_name, last_name, email, department_id, role, designation
         FROM teachers WHERE teacher_id = ?`,
        [userId]
      );
      if (rows.length > 0) profile = { ...rows[0], userType: "faculty" };
    }
