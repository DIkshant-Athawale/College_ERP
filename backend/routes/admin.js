...
router.post("/create_department", authenticate, authorize("admin"), async (req, res) => {
  ...
  try {
    ...
    if (departments.length === 0) {
      return res.status(400).json({ message: "No department data provided" });
    }
    ...
    for (const dept of departments) {
      ...
      if (!department_code || !department_name) {
        await conn.rollback();
        return res.status(400).json({ message: "department_code and department_name are required" });
      }

      const [result] = await conn.execute(
        `INSERT INTO department (department_code, department_name) VALUES (?, ?)`,
        [department_code.trim().toUpperCase(), department_name.trim()]
      );
      insertedIds.push(result.insertId);
    }

    await conn.commit();
    res.status(201).json({ message: "Department(s) created successfully", affectedRows: insertedIds.length });
  } catch (error) {
    await conn.rollback();
    console.error(error);
...
// Edit department(s)
router.patch("/departments/edit", authenticate, authorize("admin"), async (req, res) => {
  ...
  try {
    ...
    for (const dept of departments) {
      ...
      if (!department_id || !updates || Object.keys(updates).length === 0) continue;
    ...
    await conn.commit();
    res.json({ message: "Department(s) updated successfully" });
  } catch (error) {
    await conn.rollback();
    console.error(error);
    if (error.code === "ER_DUP_ENTRY") {
...
router.delete("/departments/:id", authenticate, authorize("admin"), async (req, res) => {
  ...
  try {
    ...
    // Block deletion if dept has linked teachers, students, or courses
    const [[{ teacherCount }]] = await conn.execute(
      `SELECT COUNT(*) AS teacherCount FROM teachers WHERE department_id = ?`, [id]
    );
    const [[{ studentCount }]] = await conn.execute(
      `SELECT COUNT(*) AS studentCount FROM students WHERE department_id = ?`, [id]
    );
    const [[{ courseCount }]] = await conn.execute(
      `SELECT COUNT(*) AS courseCount FROM courses WHERE department_id = ?`, [id]
    );

    if (teacherCount > 0 || studentCount > 0 || courseCount > 0) {
      await conn.rollback();
      return res.status(400).json({
        message: `Cannot delete department: it has ${teacherCount} teacher(s), ${studentCount} student(s), and ${courseCount} course(s) linked to it. Remove them first.`
      });
    }
...
router.post("/create_student", authenticate, authorize("admin"), async (req, res) => {
  ...
  try {
    ...
    for (const student of students) {
      ...
      //find eligible core courses for auto-enrollment (electives are managed by teachers)
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const course of courses) {
        ...
        const fields = [];
        const values = [];
        const validSubjectTypes = ['core', 'open', 'prof'];

        for (const key of Object.keys(updates)) {
          if (!allowedFields.includes(key)) continue;

          // Validate subject_type
          if (key === 'subject_type' && !validSubjectTypes.includes(updates[key])) {
            await conn.rollback();
            return res.status(400).json({
              message: `Invalid subject_type '${updates[key]}'. Must be one of: core, open, prof`
            });
          }
...
router.post("/create_course", authenticate, authorize("admin"), async (req, res) => {
  ...
  try {
    ...
    for (const course of courses) {
      ...
      const validTypes = ['core', 'open', 'prof'];
      const resolvedType = validTypes.includes(subject_type) ? subject_type : 'core';
      ...
      const [result] = await conn.execute(
        `INSERT INTO courses
         (course_code, course_name, department_id, year, semester, teacher_id, subject_type)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const student of students) {
        ...
        // Get new semester CORE courses only (electives managed by teachers)
        const [courses] = await conn.execute(
          `SELECT course_id
           FROM courses
           WHERE department_id = ?
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // 3️⃣ Insert fresh enrollments for that semester (core subjects only)
      await conn.execute(
        `INSERT INTO enrollments (student_id, course_id, academic_year)
         SELECT ?, course_id, ?
         FROM courses
