...
// Teachers can manually enroll/unenroll students in open/prof elective courses they teach
// ========================

// GET: enrolled + eligible students for a teacher's elective course
router.get(
  "/enrollment/:course_id/students",
  authenticate,
  authorize("faculty"),
  async (req, res) => {
    const { course_id } = req.params;
    ...
    try {
      // Verify teacher owns this course AND it's an elective
      const [courseRows] = await pool.execute(
        `SELECT course_id, course_name, course_code, subject_type, semester, department_id
         FROM courses
         WHERE course_id = ? AND teacher_id = ? AND subject_type IN ('open', 'prof')`,
        [course_id, teacher_id]
      );

      if (courseRows.length === 0) {
        return res.status(403).json({ message: "Not authorized or course is not an elective" });
      }

      const course = courseRows[0];

      // Get already enrolled students with their academic_year
      const [enrolled] = await pool.execute(
        `SELECT s.student_id, s.first_name, s.last_name, e.academic_year
         FROM enrollments e
         JOIN students s ON e.student_id = s.student_id
      ...
      // Get eligible students: same dept + semester, active, NOT yet enrolled in this course
      const [eligible] = await pool.execute(
        `SELECT s.student_id, s.first_name, s.last_name, s.academic_year
         FROM students s
         WHERE s.department_id = ?
           AND s.semester = ?
           AND s.status = 'active'
           AND s.student_id NOT IN (
             SELECT e2.student_id FROM enrollments e2 WHERE e2.course_id = ?
           )
         ORDER BY s.first_name, s.last_name`,
        [course.department_id, course.semester, course_id]
      );

      res.json({ course, enrolled, eligible });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
    }
...
// POST: enroll one or more students in a teacher's elective course
router.post(
  "/enrollment/enroll",
  authenticate,
  authorize("faculty"),
  async (req, res) => {
    ...
    if (!course_id || !Array.isArray(student_ids) || student_ids.length === 0) {
      return res.status(400).json({ message: "course_id and student_ids[] are required" });
    }
    ...
    try {
      ...
      // Verify teacher owns this elective course
      const [courseRows] = await conn.execute(
        `SELECT course_id, semester, department_id
         FROM courses
         WHERE course_id = ? AND teacher_id = ? AND subject_type IN ('open', 'prof')`,
        [course_id, teacher_id]
      );
      ...
      for (const student_id of student_ids) {
        // Get student's current academic_year
        const [studentRows] = await conn.execute(
          `SELECT academic_year FROM students WHERE student_id = ? AND status = 'active'`,
          [student_id]
        );

        if (studentRows.length === 0) continue;

        const academic_year = studentRows[0].academic_year;

        // Insert enrollment (ignore duplicates)
        await conn.execute(
          `INSERT IGNORE INTO enrollments (student_id, course_id, academic_year)
           VALUES (?, ?, ?)`,
      ...
      await conn.commit();
      res.status(201).json({ message: "Student(s) enrolled successfully" });
    } catch (error) {
      await conn.rollback();
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
...
// DELETE: remove a student from a teacher's elective course
router.delete(
  "/enrollment/unenroll",
  authenticate,
  authorize("faculty"),
